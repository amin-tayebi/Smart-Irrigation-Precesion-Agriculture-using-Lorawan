...
def saveToFile(someJSON):
	...
	# 4end of  main components 
	
	# sub of above components
	...
	now = datetime.now()
	pathNFile = now.strftime("%Y%m%d") + ".csv"
	print(pathNFile)
	if (not os.path.isfile(pathNFile)):
		with open(pathNFile, 'a', newline='') as tabFile:
			fw = csv.writer(tabFile, dialect='excel-tab')
			fw.writerow(["received_at", "application_id", "device_id", "f_port", "f_cnt", "frm_payload", "rssi", "snr", "data_rate", "consumed_airtime"])
	...
	# Application log
	pathNFile = application_id + ".csv"
	print(pathNFile)
	if (not os.path.isfile(pathNFile)):
		with open(pathNFile, 'a', newline='') as tabFile:
			fw = csv.writer(tabFile, dialect='excel-tab')
			fw.writerow(["received_at", "device_id", "f_port", "f_cnt", "frm_payload", "rssi", "snr", "data_rate", "consumed_airtime"])
	...
	# Device log
	pathNFile = application_id + "__" + device_id + ".csv"
	print(pathNFile)
	if (not os.path.isfile(pathNFile)):
		with open(pathNFile, 'a', newline='') as tabFile:
			fw = csv.writer(tabFile, dialect='excel-tab')
