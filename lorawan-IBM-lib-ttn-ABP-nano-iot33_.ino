...
//-----------------------------------------------------
struct sensorData {
  int payload[1];   // 6 payloads like 26.45  (26.45*100)
  ...
  while (resultsReceived < resultsExpected && cmd_number <= 9) {
    ...
    while (mySDI12.available()) {
      ...
      if (c == '-' || (c >= '0' && c <= '9') || c == '.') {
        ...
        levelinfo.sensor.payload[resultsReceived] = result * 1000;
        Serial.print("---->");
        Serial.print( levelinfo.sensor.payload[resultsReceived]);
        // ---------------------------------------------------------
        if (result != -9999) {
          resultsReceived++;
        }
